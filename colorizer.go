...
func (r TextRenderer) colorizeRefs(input string) string {
	...
	for index, part := range parts {
		...
		case strings.Contains(part, "HEAD"), strings.Contains(part, "master"), strings.Contains(part, "main"):
			out.WriteString(r.styler.Apply("branch", part))
		case strings.Contains(part, "tag:"):
			// Split tag reference to color version number differently
			// tagContent := part
			if idx := strings.Index(part, "tag:"); idx >= 0 {
				tagPrefix := part[:idx+4] // "tag:"
				tagName := part[idx+4:]   // everything after "tag:"
				
				// Try to extract version number from the tag
				if versionMatch := regexp.MustCompile(`^(.*?)(v?\d+\.\d+\.\d+(?:[.-][^\s,)]+)?)(.*)$`).FindStringSubmatch(tagName); versionMatch != nil {
					out.WriteString(r.styler.Apply("tag", fmt.Sprintf("%s%s%s", r.icons.Get("tag"), tagPrefix, versionMatch[1])))
					out.WriteString(r.styler.Apply("tag_version", versionMatch[2]))
					out.WriteString(r.styler.Apply("tag", versionMatch[3]))
				} else {
					out.WriteString(r.styler.Apply("tag", fmt.Sprintf("%s%s", r.icons.Get("tag"), part)))
				}
			} else {
